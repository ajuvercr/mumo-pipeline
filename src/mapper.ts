...
const qudt = createTermNamespace("http://qudt.org/1.1/schema/qudt#", "unit", "numericValue", "QuantityValue");
const qudtUnit = createTermNamespace("http://qudt.org/1.1/vocab/unit#", "DegreeCelsius");
const sosa = createTermNamespace("https://www.w3.org/ns/sosa/", "Sensor", "observes", "madeBySensor")
...
const isotc = createTermNamespace("http://def.isotc211.org/iso19156/2011/Observation#",
  "OM_Observation.resultTime", "OM_Observation.result", "OM_Observation");
const oslo = createTermNamespace("https://data.vlaanderen.be/ns/observaties-en-metingen#");
...
function observation(data: Model, key: string) {
  ...
  quads.push(quad(subj, isotc["OM_Observation.resultTime"], literal(time, XSD.terms.dateTime)));
  quads.push(quad(subj, isotc["OM_Observation.result"], resultId));
  quads.push(quad(subj, sosa.madeBySensor, mumoData.custom(`sensor/${key}/${device_id}`)));

  quads.push(quad(resultId, RDF.terms.type, qudt.QuantityValue));
  quads.push(quad(resultId, qudt.unit, typeDict[key]));
  quads.push(quad(resultId, qudt.numericValue, literal(value, XSD.terms.custom("float"))));
